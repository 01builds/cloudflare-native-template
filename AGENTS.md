...
- **API Gateway**: Hono.js inside `packages/gateway/src/index.ts`.
...
2. **Type Safety & Transport**: Router type definitions MUST be exported from `packages/gateway` (`export type AppType = typeof routes`) and consumed by `apps/web/src/lib/api.ts` via `hc<AppType>()`.
...
- `packages/gateway`: Request routing, authentication, security middleware, edge handlers.
